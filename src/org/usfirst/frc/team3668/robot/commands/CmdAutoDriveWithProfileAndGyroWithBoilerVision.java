...
public class CmdAutoDriveWithProfileAndGyroWithBoilerVision extends CmdBothDriveWithProfileAndGyro{
	private static boolean _foundTarget;
	private static double _timeAtInit;
	private static double _distanceToTarget;
	public CmdAutoDriveWithProfileAndGyroWithBoilerVision() {
	...
	public void execute(){
		if (VisionProcessing.hasFoundBoilerTarget() && !_foundTarget) {
			_foundTarget = true;
			_distanceToTarget = VisionProcessing.getBoilerCalculatedDistanceFromTarget();
			_distance = _distanceToTarget - Settings.shooterDistanceFromBoiler;
		}
		if(_foundTarget){
			super.execute();	
		}
